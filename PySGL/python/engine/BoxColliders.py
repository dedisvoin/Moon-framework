...
class MeshCollider:
    def __init__(self, vertices: list[Vector2f], material: ColliderMaterial, 
                 collider_type: ColliderType, identifier: Optional[str] = None):
        """
        Инициализация полигонального коллайдера.
        
        Args:
            vertices: Список вершин полигона (по часовой стрелке или против)
            material: Материал коллайдера
            collider_type: Тип коллайдера (STATIC, KINEMATIC, DYNAMIC)
            identifier: Уникальный идентификатор коллайдера
        """
        ...
        self._edges = []
        self._normals = []
        self._aabb = (0, 0, 0, 0)  # x_min, y_min, x_max, y_max
        self._dirty = True  # Флаг необходимости пересчета
    ...
    def _calculate_edges(self):
        """Вычисляет ребра и нормали полигона."""
        ...
        for i in range(len(self.vertices)):
            ...
            # Нормаль к ребру (перпендикулярный вектор, направленный наружу)
            normal = Vector2f(-edge.y, edge.x).normalize()
            self._normals.append(normal)
    
    def get_transformed_vertices(self) -> list[Vector2f]:
        """Возвращает вершины с учетом трансформаций (позиция, вращение, масштаб)."""
        if not self._dirty:
            return self._transformed_vertices
            
        cos_angle = math.cos(math.radians(self.rotation))
        sin_angle = math.sin(math.radians(self.rotation))
        ...
        for vertex in self.vertices:
            # Применяем масштаб
            x = vertex.x * self.scale.x
            y = vertex.y * self.scale.y
            ...
            # Применяем позицию
            x_final = x_rot + self.position.x
            y_final = y_rot + self.position.y
            
            self._transformed_vertices.append(Vector2f(x_final, y_final))
            
            # Обновляем AABB
            min_x = min(min_x, x_final)
            min_y = min(min_y, y_final)
            max_x = max(max_x, x_final)
            max_y = max(max_y, y_final)
    ...
    def get_aabb(self) -> tuple[float, float, float, float]:
        """Возвращает ограничивающий прямоугольник (AABB)."""
        if self._dirty:
            self.get_transformed_vertices()
        return self._aabb
    
    def get_edges(self) -> tuple[Vector2f]:
        """Возвращает ребра полигона."""
        return self._edges
    
    def get_normals(self) -> tuple[Vector2f]:
        """Возвращает нормали к ребрам полигона."""
        return self._normals
    
    def project_onto_axis(self, axis: Vector2f) -> tuple[float, float]:
        """
        Проецирует полигон на ось и возвращает минимальное и максимальное значения.
        
        Args:
            axis: Ось для проекции (должна быть нормализована)
            
        Returns:
            Кортеж (min, max) - минимальное и максимальное значения проекции
        """
        vertices = self.get_transformed_vertices()
        min_proj = axis.dot(vertices[0])
        max_proj = min_proj
    ...
    def check_collision(self, other: 'MeshCollider') -> CollisionInfo:
        """
        Проверяет столкновение с другим полигональным коллайдером.
        Использует алгоритм Separating Axis Theorem (SAT).
        
        Args:
            other: Другой MeshCollider
            
        Returns:
            CollisionInfo с информацией о столкновении
        """
        info = CollisionInfo(Vector2f(0, 0), 0)
        
        # Быстрая проверка AABB для раннего выхода
        aabb1 = self.get_aabb()
        ...
        # Проверка SAT по всем осям
        axes = self.get_normals() + other.get_normals()
        min_overlap = float('inf')
        smallest_axis = Vector2f(0, 0)
        ...
        for axis in axes:
            ...
            if overlap <= 0:
                return info  # Нет столкновения
                
            if overlap < min_overlap:
                min_overlap = overlap
                smallest_axis = axis
        
        # Определяем направление нормали столкновения
        ...
        info.depth = min_overlap
        info.has_collision = True
        
        return info
    
    def reset_touches(self):
        """Сбрасывает флаги касаний."""
        self.touches = {
            "left": False,
            "right": False,
            "top": False,
    ...
    def move(self, delta: Vector2f):
        """Перемещает коллайдер на указанный вектор."""
        self.position += delta
        self._dirty = True
    
    def rotate(self, angle: float):
        """Поворачивает коллайдер на указанный угол (в градусах)."""
        self.rotation += angle
        self._dirty = True
    
    def update(self, delta_time: float):
        """Обновляет состояние коллайдера."""
        if self.type == ColliderType.DYNAMIC:
            self.move(self.speed * delta_time)
            self.rotate(self.angular_velocity * delta_time)
            self._dirty = True